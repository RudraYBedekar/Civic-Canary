...
class PortalTarget(BaseModel):
    ...
    start_url: str = "fixture://benefits"
    allowed_hosts: list[str] = Field(default_factory=lambda: ["benefits.demo.local"])
    journey_steps: list[JourneyStep] = Field(
        default_factory=lambda: [
            JourneyStep(path="/index.html", label="Overview"),
            JourneyStep(path="/documents.html", label="Required documents"),
            JourneyStep(path="/apply.html", label="Application form"),
        ]
    )
...
class DetectedChange(BaseModel):
    category: Literal["REQUIREMENT", "BROKEN_LINK", "ACCESSIBILITY", "CONTENT"]
